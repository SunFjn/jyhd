...
(function (window, document, Laya) {
    ...
    var ___Laya = (function () {
        ...
        Laya.MiniAdpter = /*__JS__ */{
            init: function () {
                if (window.navigator && window.navigator.userAgent && window.navigator.userAgent.indexOf("MiniGame") > -1) console.error("请先引用小游戏适配库laya.wxmini.js,详细教程：https://ldc.layabox.com/doc/?nav=zh-ts-5-0-0")
            }
        };
        __static(Laya,
            ['conchMarket', function () {
    ...
    var EventDispatcher = (function () {
        ...
        __proto.event = function (type, data) {
            ...
            if (!list.lock) {
                ...
                while (list.cursor) {
                    ...
                    if (node.once) {
                        EventDispatcher.delNode(list, node);
                    }

                    if (data != null) {
                        if (!node.args) {
                            if (data instanceof Array) {
                                node.method.apply(node.caller, data);
                            } else {
                                node.method.call(node.caller, data);
                            }
                        } else {
                            node.method.apply(node.caller, node.args.concat(data));
                        }
                    } else {
                        node.method.apply(node.caller, node.args);
                    }

                    if (node.once) {
                        EventDispatcher.recoverHandler(node);
                    }
                }

                list.lock = false;

                if (!list.head && this._events && this._events[type] == list) {
                    delete this._events[type];
                }

                if (list.queue && list.queue.length) {
                    this.event(type, list.queue.shift());
                }
            } else {
                if (!list.queue) {
                    list.queue = [];
        ...
        __proto._createListener = function (type, caller, listener, args, once, offBefore) {
            ...
            var handler = EventDispatcher.createHandler(caller || this, listener, args, once);
        ...
        __proto.off = function (type, caller, listener, onceOnly) {
            ...
            var node = list.head;
            while (node) {
                if ((!caller || node.caller === caller) && node.method === listener && (!onceOnly || node.once)) {
                    EventDispatcher.delNode(list, node);
                    if (list.cursor == node) {
                        list.cursor = node.next;
                    }
        ...
        __proto._recoverHandlers = function (list) {
            if (!list) return;
            var node = list.head;
            while (node) {
                EventDispatcher.delNode(list, node);
        ...
        EventDispatcher.delNode = function (list, node) {
            if (node.prev) {
        ...
        EventDispatcher.createHandler = function (caller, method, args, once) {
            var result = EventDispatcher._freeHandler;
            if (result) {
                EventDispatcher._freeCount--;
                EventDispatcher._freeHandler = result.next;
                ...
                result = {
                    caller: caller, method: method, args: args, once: once
                }
            }
        ...
        EventDispatcher.recoverHandler = function (handler) {
            handler.caller = handler.method = handler.args = handler.prev = null;
            handler.next = EventDispatcher._freeHandler;
            EventDispatcher._freeHandler = handler;
            EventDispatcher._freeCount++;
        }
        EventDispatcher._freeHandler = null;
        EventDispatcher._freeCount = 0;
    ...
    var Handler = (function () {
        ...
        __proto.recover = function () {
            if (this._id > 0) {
                this._id = 0;
                this.clear();
                // this.args = Handler._freeHandler;
                // Handler._freeHandler = this;
                // ++Handler._freeCount;
            }
        }
        ...
        Handler.create = function (caller, method, args, once) {
            (once === void 0) && (once = true);
            // var result = Handler._freeHandler;
            // if (result) {
            //     Handler._freeHandler = result.args;
            //     --Handler._freeCount;
            //     return result.setTo(caller, method, args, once);
            // }
            return new Handler(caller, method, args, once);
        }

        // Handler._freeHandler = null;
        // Handler._freeCount = 0;
        Handler._gid = 1;
        return Handler;
    })()
    ...
    var MathUtil = (function () {
        ...
        MathUtil.slerpQuaternionArray = function (a, Offset1, b, Offset2, t, out, Offset3) {
            var ax = a[Offset1 + 0], ay = a[Offset1 + 1], az = a[Offset1 + 2], aw = a[Offset1 + 3], bx = b[Offset2 + 0],
                by = b[Offset2 + 1], bz = b[Offset2 + 2], bw = b[Offset2 + 3];
    ...
    var SoundManager = (function () {
        ...
        SoundManager.addChannel = function (channel) {
            ...
            if (SoundManager.autoReleaseSound && SoundManager._notUsedCount != 0) {
                if (SoundManager._notUsedUrls[channel.url]) {
                    delete SoundManager._notUsedUrls[channel.url];
                    if (--SoundManager._notUsedCount == 0) {
                        Laya.timer.clear(SoundManager, SoundManager.disposeSoundIfNotUsed);
                    }
                }
            }
        }
        ...
        SoundManager.removeChannel = function (channel) {
            var channels = SoundManager._channels;
            var i = channels.indexOf(channel);
            if (i != -1) {
                channels.splice(i, 1);
                if (SoundManager.autoReleaseSound) {
                    var url = channel.url;
                    var interval = SoundManager.defaultIntervals[url] || 20000;
                    if (interval > 0) {
                        var notUsed = true;
                        ...
                        if (notUsed) {
                            SoundManager._notUsedUrls[url] = Date.now() + interval;
                            if (SoundManager._notUsedCount++ == 0) {
                                Laya.timer.loop(10000, SoundManager, SoundManager.disposeSoundIfNotUsed);
                            }
                        }
                    }
                }
        ...
        SoundManager.disposeSoundIfNotUsed = function () {
            var notUsedUrls = SoundManager._notUsedUrls;
            if (SoundManager._notUsedCount != 0) {
                var now = Date.now();
                for (var url in notUsedUrls) {
                    if (now >= notUsedUrls[url]) {
                        delete notUsedUrls[url];
                        --SoundManager._notUsedCount;
        ...
        SoundManager.playSound = function (url, loops, complete, soundClass, startTime) {
            ...
            ;
            var tSound = SoundManager._pool[url];
            if (!soundClass) soundClass = SoundManager._soundClass;
            if (!tSound) {
                tSound = new soundClass();
                tSound.load(url);
                SoundManager._pool[url] = tSound;
            }
            ;
            var channel;
        ...
        SoundManager.playMusic = function (url, loops, complete, startTime) {
            ...
            return SoundManager._musicChannel = SoundManager.playSound(url, loops, complete, /*SoundManager._musicClass*/null, startTime);
        }
        ...
        SoundManager._pool = {};
        SoundManager.defaultIntervals = {};
        SoundManager._notUsedUrls = {};
        SoundManager._notUsedCount = 0;
        return SoundManager;
    })()
        ...
        function RenderContext(width, height, canvas) {
            ...
            this._drawSkin = function (x, y, args) {
                ...
                if (tSprite) {
                    ...
                    if (args[1]) {
                        console.log('this.ctx', tSprite, this.ctx)
                    }
    ...
    var RenderSprite = (function () {
        ...
        __proto._childs = function (sprite, context, x, y) {
            try {
                ...
                // 我写的,yyq
                console.error("精灵渲染异常![RenderSprite]!!!", sprite.name, sprite, context, x, y, "\nerror:", error);
            }
        }
    ...
    var ResourceManager = (function () {
        ...
        __static(ResourceManager,
            ['_systemResourceManager', function () {
                return this._systemResourceManager = new ResourceManager("System Resource Manager");
            }, 'currentResourceManager', function () {
                return this.currentResourceManager = ResourceManager._systemResourceManager;
            }
            ]);
    ...
    var Browser = (function () {
        ...
        Browser.__init__ = function () {
            ...
            /*__JS__ */
            window.requestAnimationFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || function (c) {
        ...
        __proto.rUTF = function (len) {
            ...
            while (this._pos_ < max) {
                ...
                if (c < 0x80) {
                    ...
                    v.push(f(((c & 0x3F) << 6) | (u[this._pos_++] & 0x7F)));
                } else if (c < 0xF0) {
                    c2 = u[this._pos_++];
                    v.push(f(((c & 0x1F) << 12) | ((c2 & 0x7F) << 6) | (u[this._pos_++] & 0x7F)));
                } else {
                    c2 = u[this._pos_++];
                    c3 = u[this._pos_++];
                    v.push(f(((c & 0x0F) << 18) | ((c2 & 0x7F) << 12) | ((c3 << 6) & 0x7F) | (u[this._pos_++] & 0x7F)));
                }
                i++;
            }
            return v.join("");
    ...
    var Stat = (function () {
        ...
        Stat.show = function (x, y, mode) {
            ...
            Stat._view[2] = { title: "DrawCall", value: "drawCall", color: "green", units: "int" };
            Stat._view[3] = { title: "CurMem", value: "currentMemorySize", color: "yellow", units: "M" };
            if (Render.isWebGL) {
                Stat._view[4] = { title: "Shader", value: "shaderCall", color: "red", units: "int" };
                if (!Render.is3DMode) {
                    Stat._view[0].title = "FPS(WebGL)";
                    Stat._view[5] = { title: "Canvas", value: "_canvasStr", color: "white", units: "int" };
                } else {
                    Stat._view[0].title = "FPS(3D)";
                    Stat._view[5] = { title: "TriFaces", value: "trianglesFaces", color: "blue", units: "int" };
        ...
        Stat.renderInfoPre = function () {
            if (Stat._canvas) {
                ...
                var gl = Render.WebGL.mainContext;
                for (var i = 0; i < Stat._view.length; i++) {
                    var one = Stat._view[i];
                    if (Stat._first) {
                        ctx.fillStyle = "white";
                    ...
                    if (!one.varying) {
                        ...
                        if (gl) {
                            switch (one.varying) {
                                case 1: {
                                    value = gl.getParameter(gl[one.value]);
                                    break;
                                }
                                case 2: {
                                    value = gl.getExtension(one.value);
                                    break;
                                }
                            }
                        }
                    ...
                    (one.units == "string") && (value = value.match(/\d+\.?\d/i));
    ...
    var Timer = (function () {
        ...
        /**返回统计信息。*/
        __proto.toString = function () {
            return "callLater:" + this._laters.length + " handlers:" + this._handlers.length + " pool:" + TimerHandler._freeCount;
        }
        ...
        Timer.__init$ = function () {
            ...
            TimerHandler = (function () {
                ...
                __proto.recover = function () {
                    this.clear();
                    this.args = TimerHandler._freeHandler;
                    TimerHandler._freeHandler = this;
                    TimerHandler._freeCount++;
                }
                ...
                TimerHandler._freeHandler = null;
                TimerHandler._freeCount = 0;
                TimerHandler.create = function () {
                    var result = TimerHandler._freeHandler;
                    if (result) {
                        TimerHandler._freeHandler = result.args;
                        --TimerHandler._freeCount;
                        result.args = null;
                        return result;
                    }
    ...
    var Node = (function (_super) {
        ...
        __proto.removeChildAt = function (index) {
            ...
            if (node) {
                var last = this._childs.length - 1;
                for (var j = index; j < last; ++j) {
                    this._childs[j] = this._childs[j + 1];
                }
                this._childs.length = last;
    ...
    var WebAudioSound = (function (_super) {
        ...
        __proto.load = function (url) {
            ...
            Laya.loader.load(url, Handler.create(this, this.onloaded), null, Loader.BUFFER, 4, false, null, true);
        ...
        __proto.onloaded = function (data) {
            ...
            this.data = data;
            WebAudioSound.buffs.push({ "buffer": this.data, "url": this.url });
            WebAudioSound.decode();
        }
        ...
        WebAudioSound._playEmptySound = function () {
            ...
            ;
            if (WebAudioSound._emptySource) {
                WebAudioSound._emptySource.stop();
                WebAudioSound._emptySource.disconnect(0);
                if (!WebAudioSoundChannel._tryCleanFailed) {
                    try {
                        WebAudioSound._emptySource.buffer = null;
                    } catch (e) {
            ...
            source.start(0, 0, 0);
            WebAudioSound._emptySource = source;
        }
        ...
        WebAudioSound._emptySource = null;
    ...
    var Loader = (function (_super) {
        function Loader() {
            ...
            this._onImageCallback = this._loadImageComplete.bind(this);
            this._onLoadedCallback = this._loadedComplete.bind(this);
        ...
        __proto.load = function (url, type, cache, group, ignoreCache) {
            ...
            if (Loader.preLoadedMap[url]) {
                ...
                base.assets.ResourcePool.instance.load(url, this._onLoadedCallback, this.priority, this.realTime);
            }
        }
        ...
        __proto._loadImage = function (url) {
            base.assets.ResourcePool.instance.load(url, this._onImageCallback, 0, true);
        }

        __proto._loadImageComplete = function (url, handle, res) {
            if (res == null) {
                this.event(/*laya.events.Event.ERROR*/"error", "Load image failed");
                return;
            }

            var view = new DataView(res);
            var image = new laya.webgl.resource.WebGLImage(new Uint8Array(res, 8), url, 0, false, view.getUint32(0), view.getUint32(4));
            this.onLoaded(image);
        }
        ...
        __proto._loadedComplete = function (url, handle, res) {
            if (res == null) {
            ...
            switch (this._type) {
                ...
                case "json":
                    ...
                    this.onLoaded(JSON.parse(bytes.readUTFBytes()));
                    break;
                case "font":
                    var bytes = new Byte(res);
                    this.onLoaded(Utils.parseXMLFromString(bytes.readUTFBytes()));
                    break;
                default:
                    this.onLoaded(res);
            }
        ...
        __proto.onLoaded = function (data) {
            ...
            if (type == "plf") {
                ...
                if (!data.src && !data._setContext) {
                    ...
                    if (!this._data.pics) {
                        throw new Error("this._data.pics == null, " + this._url);
                    }
                    ...
                    if (this._data.meta && this._data.meta.scale && this._data.meta.scale != 1) {
                        ...
                        for (var name in frames) {
                            ...
                            Loader.keepCache(this._url, directory + name);
                            tTexture.url = url;
                            map.push(url);
                        }
                    } else {
        ...
        Loader.clearRes = function (url, forceDispose) {
            ...
            if (arr) {
                for (var i = 0, n = arr.length; i < n; i++) {
                    ...
                    Loader.clearCache(url.substring(URL.basePath.length), resUrl.substring(URL.basePath.length));
        ...
        Loader.keepCache = function (obj, url) {
            if (!obj || !url) return;
            var cache = Loader.cacheSet[url];
            if (!cache) {
                Loader.cacheSet[url] = [obj];
            } else if (cache.indexOf(obj) == -1) {
                cache.push(obj);
            }
        }

        Loader.clearCache = function (obj, url) {
            if (!obj || !url) return;
            var cache = Loader.cacheSet[url];
            if (!cache) return;
            var j = cache.indexOf(obj);
            if (j === -1) {
                return;
            }
            var last = cache.length - 1;
            if (last == 0) {
                delete Loader.cacheSet[url];
        ...
        Loader.saveObject = function (url, value, recover) {
            if (!value || !url) return;
            var args = Loader.objectSet[url];
            if (args) {
                console.warn("Loader.saveObject: " + url);
                args[0] = value;
                args[1] = recover;
            } else {
                Loader.objectSet[url] = [value, recover, []];
            }
        }

        Loader.findObject = function (url) {
            if (url) {
                var args = Loader.objectSet[url];
                if (args) {
                    return args[0];
        ...
        Loader.keepObject = function (obj, url) {
            if (!obj || !url) return;
            var args = Loader.objectSet[url];
            if (args && (args[2].indexOf(obj) === -1)) {
                args[2].push(obj);
            }
        }

        Loader.clearObject = function (obj, url) {
            if (!obj || !url) return;
            var args = Loader.objectSet[url];
            if (!args) return;
            var array = args[2];
            var j = array.indexOf(obj);
            if (j === -1) {
                return;
            }
            var last = array.length - 1;
            if (last == 0) {
                delete Loader.objectSet[url];
                if (args[1]) {
                    args[1].call(null, args[0]);
                }
            } else {
                array[j] = array[last];
                array.length = last;
    ...
    var LoaderManager = (function (_super) {
        ...
        __proto._doLoad = function (resInfo) {
            ...
            if (resInfo.priority > 1) {
                loader.priority = resInfo.priority * 1800;
                loader.realTime = false;
            } else {
                loader.priority = 0;
                loader.realTime = true;
            }
        ...
        __static(LoaderManager,
            ['createMap', function () {
                return this.createMap = { atlas: [null, /*laya.net.Loader.ATLAS*/"atlas"] };
            }
            ]);
    ...
    var Resource = (function (_super) {
        ...
        Resource.getResourceByURL = function (url, index) {
            ...
            if (Resource._urlResourcesMap[url] == null) {
                return null;
            }
    ...
    var WebAudioSoundChannel = (function (_super) {
        ...
        __proto.stop = function () {
            ...
            this.completeHandler = null;
            // if (SoundManager.autoReleaseSound)
            //     Laya.timer.once(35000, null, SoundManager.disposeSoundIfNotUsed, [this.url], true);
        }
    ...
    var Stage = (function (_super) {
        ...
        __proto.setScreenSize = function (screenWidth, screenHeight) {
            ...
            this._height = (this._height + 1) & 0x7FFFFFFE;
            this._width = (this._width + 1) & 0x7FFFFFFE;
            canvasWidth = (canvasWidth + 1) & 0x7FFFFFFE;
            canvasHeight = (canvasHeight + 1) & 0x7FFFFFFE;
