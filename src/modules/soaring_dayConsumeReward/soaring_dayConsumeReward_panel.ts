...
namespace modules.soaring_dayConsumeReward {
    ...
    import SoaringDayConsumeRewardCfg = modules.config.SoaringDayConsumeRewardCfg;
    import consume_reward_fs = Configuration.consume_reward_fs;
    import Event = laya.events.Event;

    export class SoaringDayConsumeRewardPanel extends ui.SoaringDayConsumeRewardViewUI {
        ...
        protected initialize(): void {
            ...
            this._list.x = 15;
            this._list.y = 281;
            this._list.itemRender = SoaringDayConsumeRewardItem;
        ...
        protected addListeners(): void {
            ...
            this.addAutoListener(GlobalData.dispatcher, CommonEventType.SOARING_DATCONSUMEREWARD_UPDATE, this, this.showUI);
        ...
        public showUI() {
            let shuju = SoaringDayConsumeRewardCfg.instance.getCfgByType(SoaringDayConsumeRewardModel.instance.cuyType);
        ...
        public onOpened(): void {
            super.onOpened();
            SoaringDayConsumeRewardCtrl.instance.GetConsumeRewardFSInfo();
            this.showUI1();
        }
        public showUI1() {
            // 财仙猫>至尊特权>战力护符>辅助装备
        ...
        private sortFunc(a: consume_reward_fs, b: consume_reward_fs): number {
            let rewarList: Array<ConsumeRewardFSReward> = SoaringDayConsumeRewardModel.instance.rewardList;
            let aID: number = a[ConsumeRewardFSRewardFields.id];
            let bID: number = b[ConsumeRewardFSRewardFields.id];
            let aState: number = rewarList[aID] ? rewarList[aID][ConsumeRewardFSRewardFields.state] : 0;
            let bState: number = rewarList[bID] ? rewarList[bID][ConsumeRewardFSRewardFields.state] : 0;
        ...
        private setActivitiTime(): void {
            Laya.timer.clear(this, this.activityHandler);
            let isOpen = FuncOpenModel.instance.getFuncIsOpen(ActionOpenId.consumeRewardFS);
            if (SoaringDayConsumeRewardModel.instance.restTm >= GlobalData.serverTime &&
                isOpen &&
                SoaringDayConsumeRewardModel.instance.endFlag == 0) {
        ...
        private activityHandler(): void {
            this.activityText.text = `${CommonUtil.timeStampToHHMMSS(SoaringDayConsumeRewardModel.instance.restTm)}`;
            if (SoaringDayConsumeRewardModel.instance.restTm < GlobalData.serverTime) {
                this.activityText1.color = "#FF3e3e";
                this.activityText.visible = false;
                this.activityText1.text = "活动已结束";
