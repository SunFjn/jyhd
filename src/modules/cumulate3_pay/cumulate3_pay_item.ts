...
namespace modules.cumulate3_pay {
    ...
    export class CumulatePay3Item extends CumulateItemUI {
        private _cfg: cumulate_pay3;
        ...
        protected initialize(): void {
            ...
            this._itemId = 0;
            this._itemIdAr = [];
            this.cumulateText.color = "#b15315";
        ...
        protected setData(value: any): void {
            this._cfg = value as cumulate_pay3;
            let awardArr: Array<Items> = [];
            //this._stateMoney=CumulatePayModel.instance.getMoney;
            this._cumuMoney = this._cfg[cumulate_pay3Fields.money];
            this._itemId = this._cfg[cumulate_pay3Fields.id];
            this._itemIdAr.push(this._cfg[cumulate_pay3Fields.id]);
            let showIdArr: number[] = [];
            awardArr = this._cfg[cumulate_pay3Fields.reward];
        ...
        private sureBtnHandler(): void {
            if (this.getState == 0) {
                ...
                WindowManager.instance.close(WindowEnum.CUMULATE_PAY3_UPDATE);
            }
            if (this.getState == 1) {
                let rewards: Array<Items> = this._cfg[cumulate_pay3Fields.reward];
                ...
                if (BagUtil.canAddItemsByBagIdCount(items)) {
                    Cumulate3PayCtrl.instance.getCumulatepay3Reward([this._itemId]);
                }
            }
        }
        ...
        private setMoney(): void {
            this._stateMoney = CumulatePay3Model.instance.totalMoney;
            //this.priceText.text = `${this._cfg[cumulate_payFields.money]}元`;
            this.cumulateText.innerHTML = `${this._cfg[cumulate_payFields.money]}元<span style='color:#585858'>&nbsp;,可领取奖励！</span>`;
