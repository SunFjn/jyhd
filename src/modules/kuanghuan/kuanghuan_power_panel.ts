...
namespace modules.kuanghuan {
    ...
    export class KuangHuanPowerPanel extends ui.KuangHuanPowerViewUI {
        //List滑动条
        ...
        private updateItem() {
            ...
            if (nodeList) {
                for (let i: int = 0; i < nodeList.length; i++) {
                    if (nodeList[i][KuanghuanNoteFields.type] === 9) {
        ...
        private setActivitiTime(): void {
            ...
            if (this._activityTime) {
                let isOpen = modules.funcOpen.FuncOpenModel.instance.getFuncIsOpen(ActionOpenId.kuanghuan3);
