namespace modules.immortals {
    ...
    import SkillIcon2UI = ui.SkillIcon2UI;
    ...
    /** 幻武翅膀 升级技能item */
    export class Skill2Item extends SkillIcon2UI {
        ...
        public set skillInfo(value: SkillInfo) {
            ...
            if (!value) {                     // 不可激活（可激活和已激活状态服务器都会发）
                ...
                let skillCfg: skill = SkillCfg.instance.getCfgById(value[SkillInfoFields.skillId]);
                if (skillCfg) {
                    this.skillName.text = skillCfg[skillFields.name];
                    this.skillDesc.text = skillCfg[skillFields.shortDes] ? skillCfg[skillFields.shortDes] : skillCfg[skillFields.des];
                } else {
                    this.skillName.text = "技能名称";
                    this.skillDesc.text = "技能介绍";
                }
