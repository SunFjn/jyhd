...
namespace modules.cumulatePay_tianZhu {
    ...
    export class CumulatePayTianZhuItem extends ui.SoaringItemUI {
        private _taskBase: Array<BaseItem>;
        private _btnClip: CustomClip;//按钮特效
        private _Datas: single_pay_print;
        ...
        protected setData(value: single_pay_print): void {
            super.setData(value);
            this._Datas = value;
            this.showUI();
        }
        ...
        public sureBtnHandler() {
            let id = this._Datas[single_pay_printFields.id];
            let count = this._Datas[single_pay_printFields.count];
            let shuju = CumulatePayTianZhuModel.instance.rewardList[id];
            if (shuju) {
                ...
                if (param > 0) {
                    ...
                    let rewards = this._Datas[single_pay_printFields.award];
                    if (rewards) {
                        ...
                        if (BagUtil.canAddItemsByBagIdCount(items)) {
                            CumulatePayTianZhuCtrl.instance.GetSinglePayJadeAward(this._Datas[single_pay_printFields.id]);
                        }
                    }
                } else {
        ...
        public showUI() {
            ...
            let money = shuju[single_pay_printFields.money];
            let rewards = shuju[single_pay_printFields.award];
        ...
        public setBtnSure() {
            ...
            let count = this._Datas[single_pay_printFields.count];
            let shuju = CumulatePayTianZhuModel.instance.rewardList[id];
            if (CumulatePayTianZhuModel.instance.state) {
                if (shuju) {
                    if (shuju) {
                        let id = shuju[ThreeNumberFields.v1];
                        let aState: number = CumulatePayTianZhuModel.instance.getSteat(id);
                        if (aState == 0) {
        ...
        private creatEffect(): void {
            ...
            this._btnClip.pos(-6, -17, true);
            this._btnClip.scale(0.81, 1.07);
            this._btnClip.visible = false;
        }
    }
}
