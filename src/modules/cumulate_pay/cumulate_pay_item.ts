...
namespace modules.cumulate_pay {
    ...
    export class CumulatePayItem extends CumulateItemUI {
        private _cfg: cumulate_pay;
        private getState: number = 0;
        private _info: cumulate_pay;
        ...
        protected setData(value: any): void {
            this._cfg = value as cumulate_pay;
            let awardArr: Array<Items> = [];
            //this._stateMoney=CumulatePayModel.instance.getMoney;
            this._cumuMoney = this._cfg[cumulate_payFields.money];
            this._itemId = this._cfg[cumulate_payFields.id];
            this._itemIdAr.push(this._cfg[cumulate_payFields.id]);
            let showIdArr: number[] = [];
            awardArr = this._cfg[cumulate_payFields.reward];
        ...
        private sureBtnHandler(): void {
            ...
            if (this.getState == 1) {
                let rewards: Array<Items> = this._cfg[cumulate_payFields.reward];
                ...
                if (BagUtil.canAddItemsByBagIdCount(items)) {
                    CumulatePayModelCtrl.instance.getCumulatepayReward([this._itemId]);
                }
            }
        }
        ...
        private setMoney(): void {
            this._stateMoney = CumulatePayModel.instance.totalMoney;
