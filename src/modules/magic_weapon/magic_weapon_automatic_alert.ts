...
/** 宠物进自动购买*/
namespace modules.magic_weapon {
    ...
    export class MagicWeaponAutomaticPayAlert extends ui.AutomaticPayAlertUI {
        ...
        private toggleBtnHandler(): void {
            if (MagicWeaponModel.instance._bollTips) {
                MagicWeaponModel.instance._bollTips = false;
                this.toggleBtn.selected = false;
            }
            else {
                MagicWeaponModel.instance._bollTips = true;
                this.toggleBtn.selected = true;
            }
        }
        public cclose(bolll: boolean = false): void {
            super.close();
            MagicWeaponModel.instance._bollZiDong = bolll;
            GlobalData.dispatcher.event(CommonEventType.MAGIC_WEAPON_ZIDONG);
        }
        public destroy(): void {
            super.destroy();
        }
