...
namespace modules.sevenDayGift {
    ...
    import SevenDayCfg = modules.config.SevenDayCfg;
    ...
    export class SevenDayGiftPanel extends GiftViewUI {
        ...
        protected initialize(): void {
            ...
            this._dayDescArr = ["第一天", "第二天", "第三天", "第四天", "第五天", "第六天", "第七天"];
            for (let i: int = 0, len: int = this._dayTxts.length; i < len; i++) {
                this._dayTxts[i].text = this._dayDescArr[i];
            }

            this.titleTxt.text = "七日礼";
            this.titleImg.skin = "seven_sign/qirili.png";
        }
        ...
        protected addListeners(): void {
            ...
            this.addAutoListener(GlobalData.dispatcher, CommonEventType.SEVEN_DAY_INFO_UPDATA, this, this.updateInfo);
        }
        ...
        protected onOpened(): void {
            super.onOpened();
            for (let i: int = 0, len: int = this._items.length; i < len; i++) {
                this._items[i].type = 1;
                this._items[i].cfg = SevenDayCfg.instance.cfgs[i];
            }
            this.updateInfo();
        }
        ...
        private selectItem(index: int): void {
            if (index === -1) return;
            let info: UpdateSevenDay = SevenDayGiftModel.instance.sevenDayInfo;
            if (!info) return;
            ...
            let day: number = index + 1;
            let cfg: seven_day = SevenDayCfg.instance.getCfgByDay(index + 1);
            let isModel: number = cfg[seven_dayFields.isModel];
            let iconID: number = cfg[seven_dayFields.iconID];
            let isMove: number = cfg[seven_dayFields.isMove];
            this.isMoveIconImg(isMove, isModel);
            this.isMoveModelClip(isMove, isModel);
            let isTeXiao: number = cfg[seven_dayFields.isTeXiao];
            if (isTeXiao == 1) {
            ...
            this.nameTxt.text = cfg[seven_dayFields.name];

            let items: Array<Items> = SevenDayCfg.instance.getCfgByDay(day)[seven_dayFields.award];
            for (let i: int = 0; i < 3; i++) {
                this._topItems[i].dataSource = [items[i][ItemsFields.itemId], items[i][ItemsFields.count], 0, null];
            }

            //如果停留位置未领取
            let stateInfo: GiftState = SevenDayGiftModel.instance.getDataByIndex(index);
            if (stateInfo && stateInfo[GiftStateFields.state] === 1) {
                ...
                if (day <= info[UpdateSevenDayFields.day]) {
        ...
        public soaringRankModelClip(iconID: number, isModel: number) {
            ...
            // this._modelClip.y = 820;
            this._skeletonClip.visible = true;
            this._skeletonClip.pos(360, 820, true);
            this._skeletonClip.rotation = 0;
            ...
            if (isModel == 1) {
                ...
                if (typeNum == 2) {  //宠物
                    ...
                    this._skeletonClip.pos(370, 810);
                    this._skeletonClip.rotation = -14;
                } else if (typeNum == 90) { //时装
                    this._skeletonClip.resetScale(AvatarAniBigType.clothes, 1);
                } else if (typeNum == 11) { //灵珠
                    this._skeletonClip.y = 980;
                    // if (this._modelClip) {
        ...
        protected sureFunc(): void {
            ...
            SevenDayGiftCtrl.instance.getSevenDayAward(this._currSelectIndex + 1);
        }
        ...
        private updateInfo(): void {
            ...
            let index: number = 6;
            for (let i: int = 0, len: int = this._items.length; i < len; i++) {
                let data: GiftState = SevenDayGiftModel.instance.getDataByIndex(i);
        ...
        private effectInit(): void {
            ...
            this._btnClip.pos(this.sureBtn.x - 5, this.sureBtn.y - 16);
            ...
            this._prizeEffect.pos(104, 530);//750
        }
