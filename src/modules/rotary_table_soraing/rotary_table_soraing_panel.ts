...
namespace modules.rotary_table_soraing {
    ...
    export class RotaryTableSoaringPanel extends ui.RotaryTableSoaringViewUI {
        ...
        // private xuanZhonImgPos: Array<Array<number>>;
        constructor() {
            super();
        }
        protected initialize(): void {
            super.initialize();
            // this.xuanZhonImgPos = new Array<Array<number>>();
            ...
            //     [485, 646, 150]
            // ];
            this._proCtrl = new ProgressBarCtrl(this.blessImg, 340, this.blessTxt);
            this.initializeClip();
        ...
        protected addListeners(): void {
            ...
            this.addAutoListener(GlobalData.dispatcher, CommonEventType.ROTARYTABLE_SOARING_UPDATE, this, this.updateUI);
            this.addAutoListener(GlobalData.dispatcher, CommonEventType.ROTARYTABLE_SOARING_RUNREPLY, this, this.PayRewardRunReply);
            this.addAutoListener(GlobalData.dispatcher, CommonEventType.ROTARYTABLE_SOARING_OPENRECORD, this, this.openMyRecord);
            this.addAutoListener(GlobalData.dispatcher, CommonEventType.ROTARYTABLE_SOARING_BROADCAST_LIST, this, this.updateSeverList);
            this.addAutoListener(GlobalData.dispatcher, CommonEventType.ROTARYTABLE_SOARING_SHOWHTML, this, this.showHtml);
        }
        ...
        public probabilityNoticeFun() {
            let param: number;
            param = modules.soaring_rank.SoaringRankModel.instance.curType;

            switch (param) {
                case 1:
                    CommonUtil.alertHelp(74509);
                    break;
                case 2:
                    CommonUtil.alertHelp(74510);
                    break;
                case 3:
                    CommonUtil.alertHelp(74511);
                    break;
                case 4:
                    CommonUtil.alertHelp(74512);
                    break;
                case 5:
                    CommonUtil.alertHelp(74513);
                    break;
                case 6:
                    CommonUtil.alertHelp(74514);
                    break;
                case 7:
                    CommonUtil.alertHelp(74515);
                    break;
                default:
                    break;
            }
        ...
        public rankBtnHandler() {
            WindowManager.instance.open(WindowEnum.ROTARYTABLE_SOARING_RANKAWARD_ALERT);
        }

        protected onOpened(): void {
            super.onOpened();
            this.titleTxt.y = 54;
            this.titleTxt.color = "#ab2b1c";
            this._bolll = false;
            this._bolllPos = false;
            this._index = 0;//全服记录修改
            this._index1 = 0;//全服记录修改
            RotaryTableSoaringCtrl.instance.GetDuobaoInfo();
            RotaryTableSoaringCtrl.instance.GetDuobaoServerBroadcast();
            this.oneMoneytext.text = `${RotaryTableSoaringModel.instance._oneMoney}`;
            this.tenMoneytext.text = `${RotaryTableSoaringModel.instance._tenMoney}`;
        ...
        public updateUI() {
            ...
            this.changproUI();
            // this.changDrawNum();
            this.rankBtn.visible = RotaryTableSoaringModel.instance.type == 1;

            if (RotaryTableSoaringModel.instance.type == 1) {
                this.titleTxt.text = "封神夺宝";
            }
            else {
                this.titleTxt.text = "冲榜夺宝";
            }
            let ingot = PlayerModel.instance.ingot;//玩家代币券
            if (ingot < RotaryTableSoaringModel.instance._oneMoney) {
                this.oneMoneytext.color = "#FF3e3e";
            }
            else {
                this.oneMoneytext.color = "#ffffff";
            }
            if (ingot < RotaryTableSoaringModel.instance._tenMoney) {
        ...
        private setActivitiTime(): void {
            Laya.timer.clear(this, this.activityHandler);
            let isOpen = FuncOpenModel.instance.getFuncIsOpen(ActionOpenId.duobao);
            if (RotaryTableSoaringModel.instance.restTm >= GlobalData.serverTime &&
                isOpen) {
                this.activityText1.color = "#ffffff";
                this.activityText.visible = true;
                this.activityText1.text = "活动倒计时:";
                this.activityText1.x = 319;
                ...
                this.activityText1.x = 395;
            }
        }
        private activityHandler(): void {
            this.activityText.text = `${CommonUtil.timeStampToHHMMSS(RotaryTableSoaringModel.instance.restTm)}`;
            if (RotaryTableSoaringModel.instance.restTm < GlobalData.serverTime) {
                this.activityText1.color = "#FF3e3e";
                this.activityText.visible = false;
                this.activityText1.text = "活动已结束";
        ...
        public setARR() {
            if (!this._htmlArr) {
                ...
                for (let index = 0; index < 20; index++) {
                    let _html = new HtmlReward(270, 20, this._htmlArr);
            ...
            if (this._sevStringArr) {
                if (this._sevStringArr.length > 0) {
                    ...
                    if (_html) {
                        ...
                        _html.setText(210 + 8, 210, this.getSevStringArr());
                        if (!this._bolllPos) {
                            this._bolllPos = true;
        ...
        private updateSeverList() {
            let svrList = RotaryTableSoaringModel.instance.svrBroadcastList;
                ...
                if (svrList.length > 0) {
                    ...
                    for (let i = 0; i < 10; i++) {
                        if (svrList[i]) {
                            ...
                            html += `<a href=""><span style='color:${itemColor};'>${itemName}*${count}</span></a>`;
        ...
        public PayRewardRunReply() {
            ...
            let noteList = RotaryTableSoaringModel.instance.getNoteList();
            if (noteList.length > 1) {
                // isTen = true;
                WindowManager.instance.open(WindowEnum.ROTARYTABLE_SOARING_REWARD_ALERT, noteList);
            } else {
                //播放动画
        ...
        public showInSmoking(grade: number, isTen: boolean) {
            ...
            let _weightCfgs = DuobaoWeightCfg.instance.getDate(RotaryTableSoaringModel.instance.type, RotaryTableSoaringModel.instance.param);
            let weight = _weightCfgs[duobao_weightFields.weight];
            for (let i = 0; i < weight.length; i++) {
                let index: number = weight[i][PayRewardWeightNodeFields.index];
        ...
        public ani() {
            ...
            // let posArr = this.xuanZhonImgPos[this._nowTargetIdnex];
            if (bagItem) {
                // bagItem.addChild(this.xuanZhonImg);
                // let x = posArr[0];
                // let y = posArr[1];
                // this.xuanZhonImg.pos(x, y);
                // this.xuanZhonImg.rotation = posArr[2];
                this.xuanZhonImg.rotation = this._nowTargetIdnex * 30;
            ...
            if (this._quanNum > 0) {
                ...
                if (this._nowTargetIdnex === this._targetIdnex) {
                    ...
                    if (this._isTen) {
                        ...
                        if (this._arr) {
                            GlobalData.dispatcher.event(CommonEventType.ROTARYTABLE_SOARING_EFFECT, [this._arr]);
                            CommonUtil.delayedPutInBag();
                            let allItemId = RotaryTableSoaringModel.instance.getNoteListGj();//所有符合弹窗的奖励
        ...
        public showReward() {
            ...
            let weight = _weightCfgs[duobao_weightFields.weight];
            for (let i = 0; i < weight.length; i++) {
                let itemId: number = weight[i][PayRewardWeightNodeFields.itemId];
                let count: number = weight[i][PayRewardWeightNodeFields.count];
                let bagItem: BaseItem = this._showItem[i];
                if (bagItem) {
         ...
         */
        public changproUI() {
            let getgRade: [number, number] = RotaryTableSoaringModel.instance.getgRade();
            let typeNum = RotaryTableSoaringModel.instance.type;
            let paramNum = RotaryTableSoaringModel.instance.param;
            let _duobaoRewardCfg = DuobaoRewardCfg.instance.getDate(typeNum, paramNum, getgRade[0]);
            let conditionNUm = 0;
            if (_duobaoRewardCfg) {
                conditionNUm = _duobaoRewardCfg[duobao_rewardFields.condition];
            }
            let stateNUm = getgRade[1];
            this._proCtrl.maxValue = conditionNUm;
            this._proCtrl.value = RotaryTableSoaringModel.instance.score;
            if (stateNUm == 1) {
                this._isReward = true;
        ...
        public changDrawNum() {
            ...
            } else if (ingot > RotaryTableSoaringModel.instance._oneMoney && ingot < RotaryTableSoaringModel.instance._tenMoney) {
        ...
        private xunBaoHandler(value: number): void {
            if (this._isUpdateBtnClick) {
                ...
                if (!this._isAniIng) {
                    RotaryTableSoaringCtrl.instance.DuobaoRun(value);
                }
            } else {
        ...
        public caiQiReceiveBtnHandler(): void {
            if (this._isReward) {
                RotaryTableSoaringCtrl.instance.GetDuobaoReward();
            } else {
                //打开财气值奖励界面
                // console.log("打开财气值奖励界面");
                WindowManager.instance.open(WindowEnum.ROTARYTABLE_SOARING_RANK_ALERT, 0);
            }
        }
        /**
         * 请求我的抽奖记录
        ...
        public myRecordBtnHandler() {
            RotaryTableSoaringCtrl.instance.GetDuobaoNotes();
        }
        /**
         * 打开我的抽奖记录弹窗
         */
        public openMyRecord() {
            if (RotaryTableSoaringModel.instance.PayRewardNoteList == undefined) {
                SystemNoticeManager.instance.addNotice("暂无记录", true);
                return;
            }
            if (RotaryTableSoaringModel.instance.PayRewardNoteList.length > 0) {
                WindowManager.instance.open(WindowEnum.ROTARYTABLE_SOARING_MYRECORD_ALERT, 0);
            } else {
                SystemNoticeManager.instance.addNotice("暂无记录", true);
            }
        }
        public close(): void {
            ...
            this._prizeEffect.visible = false;
            CommonUtil.delayedPutInBag();
            WindowManager.instance.close(WindowEnum.ROTARYTABLE_SOARING_RANK_ALERT);
            WindowManager.instance.close(WindowEnum.ROTARYTABLE_SOARING_MYRECORD_ALERT);
            WindowManager.instance.close(WindowEnum.ROTARYTABLE_SOARING_REWARD_ALERT);
            WindowManager.instance.close(WindowEnum.ROTARYTABLE_SOARING_RANKAWARD_ALERT);
        }
        ...
        public initializeClip() {
            ...
            this._oneBtnBtnClip.pos(-4, -20, true);
            this._oneBtnBtnClip.scale(1, 1.3, true);
            this._oneBtnBtnClip.visible = false;
            ...
            this._tenBtnBtnClip.pos(-4, -20, true);
            ...
            this._prizeEffect.pos(443, 760, true);
        }
    }
}
