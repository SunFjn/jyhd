...
///<reference path="behavior/chase_and_attack2.ts"/>
///<reference path="action/revenge.ts"/>
///<reference path="action/patrol.ts"/>
///<reference path="action/kill_monster.ts"/>
namespace game.role.component.brain {
    ...
    import AlwaysSuccess = game.ai.behavior.decorator.AlwaysSuccess;
    ...
    import Patrol = game.role.component.brain.action.Patrol;
    import PlayerChaseAndAttack = game.role.component.brain.behavior.PlayerChaseAndAttack;
    import KillMonster = game.role.component.brain.action.KillMonster;
    import KillMonster2 = game.role.component.brain.action.KillMonster;
    ...
    import ChaseAndAttack2 = game.role.component.brain.action.ChaseAndAttack2;
    const enum TopBrainPriority {
        Death,
        Transmit,
        Abnormal,
        TargetChange,
    ...
    export class XuanhuoComponent extends RoleComponent {
        ...
        constructor(owner: Role, selectTargetPos: () => Pos, getBossOnwer: (occ: number) => number) {
            ...
            this._tree = BehaviorTree.create(
                new SelectorMonitor(this._interrupt).addChild(
                    new EventNode(new DeathAction(owner), this._interrupt, TopBrainPriority.Death)
                ).addChild(
                    new EventNode(new AbnormalAction(owner), this._interrupt, TopBrainPriority.Abnormal)
                ).addChild(
                    new EventNode(new NpcTriggerAction(owner), this._interrupt, TopBrainPriority.TargetChange)
                ).addChild(
                    new BossChaseAndAttack(owner, 600, selectTargetPos, getBossOnwer)
                ).addChild(
                    new Sequence().addChild(new ChaseAndAttack2(owner, 1000, SelectTargetType.Player)).addChild(new Patrol(owner, 900, SelectTargetType.Player))
                ).addChild(
                    new Sequence().addChild(new ChaseAndAttack(owner, 1000, SelectTargetType.Monster)).addChild(new Patrol(owner, 1000, SelectTargetType.Monster))
                ).addChild(
        ...
        private transmit(x: number, y: number): void {
            ...
            this._interrupt.trigger(TopBrainPriority.Transmit);
        }
        ...
        private interrupt(status: number): void {
            ...
            if (status == RoleState.DEATH || status == RoleState.REVIVE) {
                this._interrupt.trigger(TopBrainPriority.Death);
            }
        }

        private onTargetChange(): void {
            this._interrupt.trigger(TopBrainPriority.TargetChange);
        }

        private updateAbnormal(state: number): void {
            if (state & ActorState.dizz) {
                this._interrupt.trigger(TopBrainPriority.Abnormal);
            }
