...
///<reference path="action/situAttack.ts"/>
///<reference path="action/revenge.ts"/>
///<reference path="action/patrol.ts"/>
///<reference path="behavior/goto_assignPoint.ts"/>
...
namespace game.role.component.brain {
    ...
    import SituAttack = game.role.component.brain.action.SituAttack;
    import MoveToSpawnpoint = game.role.component.brain.behavior.MoveToSpawnpoint;
    import GotoAssignPoint = game.role.component.brain.action.GotoAssignPoint;
    ...
    export class TeamBattleComponent extends RoleComponent {
        ...
        constructor(owner: Role, selectTargetPos: () => Pos) {
            ...
            this._tree = BehaviorTree.create(
                new SelectorMonitor(this._interrupt).addChild(
                    ...
                    new GotoAssignPoint(owner, selectTargetPos)
                ).addChild(
                    //new EventNode(new AlwaysSuccess(new NpcTriggerAction(owner)), this._interrupt, TopBrainPriority.TargetChange)
                    // new Sequence().addChild(new NpcTriggerAction(owner))
                    new EventNode(new AlwaysSuccess(new NpcTriggerAction(owner)), this._interrupt, TopBrainPriority.TargetChange)
                    //new EventNode(new NpcTriggerAction(owner), this._interrupt, TopBrainPriority.TargetChange)
                ).addChild(
                    new Sequence().addChild(new SituAttack(owner, 1000, SelectTargetType.Player))
                    // .addChild(new Patrol(owner, 900, SelectTargetType.Player))
                ).addChild(
