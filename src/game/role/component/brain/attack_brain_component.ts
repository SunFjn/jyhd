...
namespace game.role.component.brain {
    ...
    const enum AttackBrainPriority {
        Death,
        Transmit,
        Abnormal,
        PosUpdate,
    ...
    export class AttackBrainComponent extends RoleComponent {
        ...
        constructor(owner: Role) {
            ...
            this._tree = BehaviorTree.create(
                new SelectorMonitor(this._interrupt).addChild(
                    new EventNode(new DeathAction(owner), this._interrupt, AttackBrainPriority.Death)
                ).addChild(
                    new EventNode(new AbnormalAction(owner), this._interrupt, AttackBrainPriority.Abnormal)
                ).addChild(
        ...
        private transmit(x: number, y: number): void {
            ...
            this._interrupt.trigger(AttackBrainPriority.Transmit);
        }

        private interrupt(status: number): void {
            if (status == RoleState.DEATH) {
                this._interrupt.trigger(AttackBrainPriority.Death);
            }
        }

        private onTargetChange(): void {
            this._interrupt.trigger(AttackBrainPriority.TargetChange);
        }

        private onPosUpdate(): void {
            this._interrupt.trigger(AttackBrainPriority.PosUpdate);
        }

        private updateAbnormal(state: number): void {
            if (state & ActorState.dizz) {
                this._interrupt.trigger(AttackBrainPriority.Abnormal);
            }
